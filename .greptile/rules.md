...
| `SearchableDropdown.js` | `_writeEditableValue()` sets an editable dropdown's wrapped `<input>` through the prototype's `value` setter and fires `input` as well as `change` | A value the user types or picks never reaches a React-controlled `<input>`, and React puts the old one back |
