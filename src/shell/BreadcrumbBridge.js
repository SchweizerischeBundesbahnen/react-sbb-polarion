 ...
 * page as a classic <script> so that `document.currentScript` is available to read its config. It is
 * therefore NOT part of this library's ES bundle — `vite.bridge.config.ts` emits it separately as
 * `dist/breadcrumb-bridge.js`, which the consuming extension copies into its own served app folder.
 * `BreadcrumbInjector` then loads it from there by URL. See "Breadcrumb bridge" in the README.
 *
 * Two ways to use it:
