 ...
 * the extension via data-* attributes; the bridge swaps the breadcrumb for "[icon] {title}" while the
 * app URL is active and never touches Polarion's own Administration pages. Renders nothing.
 *
 ...
 */
export default function BreadcrumbInjector({ marker, title, icon, src }: Readonly<BreadcrumbInjectorProps>) {
  useEffect(() => {
    try {
      ...
      if (bridge) {
        bridge.install({ marker, title, icon });
        return;
      }
