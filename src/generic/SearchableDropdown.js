...
export default class SearchableDropdown {
    ...
    _render() {
        ...
        // react-sbb-polarion patch (not in upstream generic): append the portal to the control's own
        // root node. If the control lives inside a shadow root (an extension encapsulating its styles
    ...
    _bindEvents() {
        ...
        this._outsideClickHandler = e => {
            // react-sbb-polarion patch: use composedPath() so the inside-check works when the control
            // lives in a shadow root. A document-level listener sees a retargeted e.target at the shadow
            // boundary, so container.contains(e.target) would misfire and wrongly close the popup.
            // Falls back to contains() where composedPath is unavailable.
            const path = typeof e.composedPath === 'function' ? e.composedPath() : null;
            const inside = path
                ? path.includes(this.container) || path.includes(this.portal)
                : this.container.contains(e.target) || this.portal.contains(e.target);
            if (!inside) {
                this._close();
            }
        };
